...
from keras.layers.normalizatin import BatchNormalization
...
class DeeperGoogLeNet:
    ...
    def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce, num5x5, num1x1Proj, chanDim, stage, reg=0.0005):
        ...
        first = DeeperGoogLeNet.conv_module(x, num1x1, 1, 1, (1, 1), chanDims, reg=reg, name=stage + '_first')
        ...
        @staticmethod
        def build(width, height, classes, reg=0.0005):
            ...
            x = MaxPooling2D((3, 3), strifdes=(2, 2), padding='same', name='pool1')(x)
            ...
            x = MaxPooling2D((3, 3), strifdes=(2, 2), padding='same', name='pool2')(x)

            # apply two Inception modules followed by a POOL
            x = DeeperGoogLeNet.inception_module(x, 64, 96, 128, 16, 32, 32, chanDim, '3a', reg=reg)
            x = DeeperGoogLeNet.inception_module(x, 128, 128, 192, 32, 96, 64, chanDim, '3a', reg=reg)
            x = MaxPooling2D((3, 3), strifdes=(2, 2), padding='same', name='pool3')(x)
