...
def convolve(image, K):
    # grab the spatial dimentions of the image and kernel
    (iH, iW) = image.shape[:2]
    (kH, kW) = K.shape[:2]
    print('Shape of kernel is', K.shape[:2])
    ...
    for y in np.arange(pad, iH + pad):
        for x in np.arange(pad, iW + pad):
            ...
            # of the image by extracting the
            ...
            # store the convolbed value in the output(x, y)-
...
# construct the Laplacian kernel used to detect edge-like regions in the image
...
sobelY = np.array(([-1, -2, -1], [0, 0, 0], [-1, 0, 1]), dtype='int')

# construct an amboss kernel
emboss = np.array(([-2, -1, 0], [-1, 1, 1], [0, 1, 2]), dtype='int')

# construct the kernel bank, a list of kernels we're going to apply
# using both our custom 'convolw' function ond OpenCV's 'filter2D'
# function
